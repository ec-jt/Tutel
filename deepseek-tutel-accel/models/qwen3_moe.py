...
is_triton_supported = (IS_HIP_EXTENSION and torch.cuda.get_device_properties().gcnArchName.split(':')[0] >= '90a') or \
...
def modeling(is_training=False):
  cos_sin = fetch_url(f'/tmp/qwen3_cos_sin.npy', 'https://huggingface.co/datasets/ghostplant/data-collections/resolve/main/qwen3_cos_sin.npy')
  cos_sin = cos_sin.bfloat16().to(device)
  ...
  def attn_decode_fwd(q, k_buffer, v_buffer, kv_indptr, sm_scale=None):
    ...
    if not is_triton_supported:
      ...
      if is_training:
        ...
        qk = ops.scaled_softmax_inv(qk, kv_indptr, sm_scale)
  ...
  @torch.compiler.disable(recursive=True)
  def gemm(x, y):
    return ops.gemm_nt_bf16xfp8_block_scal(x, y, getattr(y, 'scale_inv', y), 0)
  ...
  def forward_fn(token_in, token_range, logits, options):
    ...
    for l in range(n_layers):
      ...
      scores = attn_decode_fwd(q_states, key_cache[l], val_cache[l], token_range)
      xb = gemm(scores.flatten(2), o_proj[l])

      if world_size > 1:
        x = ops.x_add_allreduce_y_bf16(x, xb, True)
