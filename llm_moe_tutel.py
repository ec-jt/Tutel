...
try:
  ...
  if 'OP_LOADER' not in os.environ:
    if not IS_HIP_EXTENSION:
      ...
      # Define precompiled architectures
      PRECOMPILED_ARCHS = {"sm_80", "sm_86", "sm_90", "sm_100"}
      
      if arch_tag in PRECOMPILED_ARCHS:
        # Use existing precompiled path logic
        if cc_major == 6:
          suffix = 'p100'
        elif cc_major == 7:
          suffix = 'v100'
          ...
          # Fallback to JIT for unknown but precompiled architectures
          suffix = 'jit'
          logging.info("Using JIT - precompiled kernels for %s not found", arch_tag)
        os.environ['OP_LOADER'] = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"ops.{suffix}")
      else:
        # Use JIT compilation for unknown architectures
        ops_dir = "jit"
        logging.info("Using JIT - precompiled kernels for %s not found", arch_tag)
        os.environ['OP_LOADER'] = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"ops.{ops_dir}")
    else:
