...
try:
  ...
  # Cross-node GPU collectives via NCCL; single model group spans ALL ranks
  ...
  # NCCL can't reduce CPU tensors; use a CUDA buffer for barriers.
  _barrier_buf = torch.ones(1, dtype=torch.int32, device=device)
  def peer_barrier():
    dist.all_reduce(_barrier_buf, op=dist.ReduceOp.SUM)
...
# Cross-node residual add + all-reduce (SUM, no scaling)
def intra_add_allreduce(x: torch.Tensor, xb: torch.Tensor) -> torch.Tensor:
  y = x + xb
  # NCCL backend is initialized above; this works across nodes
...
if model_type in ('deepseek_v3', 'kimi_k2'):
  ...
  module = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"./models/qwen3_moe_dist.py")
elif model_type in ('gpt_oss'):
...
# Control-plane tensors must live on CUDA when backend='nccl'
prompt_cnt = torch.empty([3], dtype=torch.int64, device=device)
tokens_buf = torch.empty([args.max_seq_len], dtype=torch.int64, device=device)
  ...
  async def stream_chat(request: Request):
    ...
    async def generate_data():
      async with app.global_lock:
        ...
        tokens = token_encode(prompt_user.strip(), system_prompt=prompt_system).to(device)
        ...
        # Use NCCL broadcast (CUDA tensors)
        dist.broadcast(prompt_cnt, src=0)
        ...
        app.task_pool = generate_partial(tokens, max_tokens=max_tokens, temperature=temperature, screen_display=not enable_stream)
        response_buffers = []
...
''')
    ...
    with torch.no_grad():
      while True:
        #net.simple_broadcast(prompt_cnt)
        # receive CUDA control tensors via NCCL
        dist.broadcast(prompt_cnt, src=0)
